...
contract TestHelperAssert is Test, HelperAssert, ErrAllowTestHelper {
     ...
     */
    function testFuzz_errAllow_selector_array_boundaries(uint8 arraySize) public {
        // Limit array size to reasonable bounds for gas efficiency
        vm.assume(arraySize > 0 && arraySize <= 20);

        // Create array of allowed selectors
        bytes4[] memory allowedErrors = new bytes4[](arraySize);
        for (uint256 i = 0; i < arraySize; i++) {
            allowedErrors[i] = bytes4(uint32(i + 1)); // Selectors 0x00000001, 0x00000002, etc.
        }

        // Test valid selector - always test first element (guaranteed to exist)
        bytes4 validSelector = bytes4(uint32(1)); // First selector (0x00000001)
        errAllow(validSelector, allowedErrors, "fuzz array boundaries valid test");

        // Test valid selector - test last element
        bytes4 lastValidSelector = bytes4(uint32(arraySize)); // Last selector
        errAllow(lastValidSelector, allowedErrors, "fuzz array boundaries last valid test");

        // Test invalid selector not in array (guaranteed > arraySize since max arraySize is 20)
        bytes4 invalidSelector = bytes4(uint32(arraySize + 1000));
        vm.expectEmit(false, false, false, true);
        emit AssertFail("fuzz array boundaries invalid test");
        vm.expectRevert(PlatformTest.TestAssertFail.selector);
        errAllow(invalidSelector, allowedErrors, "fuzz array boundaries invalid test");
    }

    function testFuzz_errAllow_selector_position(uint8 selectorIndex) public {
        // Create array with known selectors
        bytes4[] memory allowedErrors = new bytes4[](5);
        allowedErrors[0] = bytes4(0x11111111);
        allowedErrors[1] = bytes4(0x22222222);
        allowedErrors[2] = bytes4(0x33333333);
        allowedErrors[3] = bytes4(0x44444444);
        allowedErrors[4] = bytes4(0x55555555);

        // Use selectorIndex to determine test behavior
        if (selectorIndex < 10) {
            // 10/256 chance (~4%) - test valid selectors from the array
            uint8 validIndex = selectorIndex % 5; // Cycle through valid indices 0-4
            bytes4 validSelector = allowedErrors[validIndex];
            errAllow(validSelector, allowedErrors, "fuzz selector position valid test");
        } else {
            // 246/256 chance (~96%) - test invalid selectors
            // Generate invalid selector (0x66666666 + offset) that cannot match allowed selectors
            bytes4 invalidSelector = bytes4(uint32(0x66666666 + selectorIndex));
            vm.expectEmit(false, false, false, true);
            emit AssertFail("fuzz selector position invalid test");
            vm.expectRevert(PlatformTest.TestAssertFail.selector);
            errAllow(invalidSelector, allowedErrors, "fuzz selector position invalid test");
        }
    }

    function testFuzz_errAllow_require_message_matching(uint8 messageChoice, string memory randomSuffix) public {
        // Use known allowed messages from setup
        string[] memory allowedMessages = setup_errAllow_require_error();

        string memory testMessage;
        bool shouldPass;

        if (messageChoice < 20) {
            // 20/256 chance (~8%) - test valid messages from allowedMessages
            uint8 validIndex = messageChoice % uint8(allowedMessages.length);
            testMessage = allowedMessages[validIndex];
            shouldPass = true;
        } else {
            // 236/256 chance (~92%) - test invalid messages
            testMessage = string(abi.encodePacked("invalid_message_", randomSuffix));
            shouldPass = false;
        }

        // Create error data with the test message
        bytes memory errorData = abi.encodeWithSelector(bytes4(0x08c379a0), testMessage);

        if (shouldPass) {
            errAllow(errorData, allowedMessages, "fuzz message matching valid test");
        } else {
            vm.expectEmit(false, false, false, true);
            emit AssertFail("fuzz message matching invalid test");
            vm.expectRevert(PlatformTest.TestAssertFail.selector);
            errAllow(errorData, allowedMessages, "fuzz message matching invalid test");
        }
    }
    ...
    function testFuzz_errAllow_combined_random_inputs(uint8 testType, uint8 choice, string memory randomSuffix)
        public
    {
        string[] memory allowedRequireErrors = setup_errAllow_require_error();
        bytes4[] memory allowedCustomErrors = setup_errAllow_custom_error();
        ...
        if (testType < 128) {
            // ~50% - Test require errors (Error(string) selector 0x08c379a0)
            bytes4 requireSelector = bytes4(0x08c379a0);
            string memory message;
            bool shouldPass;

            if (choice < 20) {
                // ~8% of require tests - use valid message
                uint8 validIndex = choice % uint8(allowedRequireErrors.length);
                message = allowedRequireErrors[validIndex];
                shouldPass = true;
            } else {
                // ~92% of require tests - use invalid message
                message = string(abi.encodePacked("invalid_", randomSuffix));
                shouldPass = false;
            }

            bytes memory errorData = abi.encodeWithSelector(requireSelector, message);
            if (shouldPass) {
                errAllow(errorData, allowedRequireErrors, allowedCustomErrors, "fuzz combined require valid test");
            } else {
                vm.expectEmit(false, false, false, true);
                emit AssertFail("fuzz combined require invalid test");
                vm.expectRevert(PlatformTest.TestAssertFail.selector);
                errAllow(errorData, allowedRequireErrors, allowedCustomErrors, "fuzz combined require invalid test");
            }
        } else {
            // ~50% - Test custom errors
            bytes4 selector;
            bool shouldPass;

            if (choice < 20) {
                // ~8% of custom tests - use valid selector
                uint8 validIndex = choice % uint8(allowedCustomErrors.length);
                selector = allowedCustomErrors[validIndex];
                shouldPass = true;
            } else {
                // ~92% of custom tests - use invalid selector (0x77777777 + offset)
                selector = bytes4(uint32(0x77777777 + choice));
                shouldPass = false;
            }

            bytes memory errorData = abi.encodeWithSelector(selector, randomSuffix);
            if (shouldPass) {
                errAllow(errorData, allowedRequireErrors, allowedCustomErrors, "fuzz combined custom valid test");
            } else {
                vm.expectEmit(false, false, false, true);
                emit AssertFail("fuzz combined custom invalid test");
                vm.expectRevert(PlatformTest.TestAssertFail.selector);
                errAllow(errorData, allowedRequireErrors, allowedCustomErrors, "fuzz combined custom invalid test");
            }
        }
    }

    function testFuzz_errAllow_zero_selector_edge_cases(uint8 selectorChoice) public {
        bytes4[] memory allowedErrorsWithZero = new bytes4[](3);
        allowedErrorsWithZero[0] = bytes4(0x00000000); // Zero selector
        allowedErrorsWithZero[1] = bytes4(0x11111111);
        allowedErrorsWithZero[2] = bytes4(0x22222222);

        bytes4[] memory allowedErrorsWithoutZero = new bytes4[](2);
        allowedErrorsWithoutZero[0] = bytes4(0x11111111);
        allowedErrorsWithoutZero[1] = bytes4(0x22222222);

        bytes4 selector;
        bool shouldPassWithZero;
        bool shouldPassWithoutZero;

        if (selectorChoice < 15) {
            // ~6% - Test valid selectors from allowedErrorsWithZero
            uint8 validIndex = selectorChoice % 3;
            selector = allowedErrorsWithZero[validIndex];
            shouldPassWithZero = true;
            shouldPassWithoutZero = (validIndex > 0); // Zero selector (index 0) not in second array
        } else {
            // ~94% - Test invalid selectors (0x88888888 + offset)
            selector = bytes4(uint32(0x88888888 + selectorChoice));
            shouldPassWithZero = false;
            shouldPassWithoutZero = false;
        }

        // Test with array that includes zero selector
        if (shouldPassWithZero) {
            errAllow(selector, allowedErrorsWithZero, "fuzz zero included valid test");
        } else {
            vm.expectEmit(false, false, false, true);
            emit AssertFail("fuzz zero included invalid test");
            vm.expectRevert(PlatformTest.TestAssertFail.selector);
            errAllow(selector, allowedErrorsWithZero, "fuzz zero included invalid test");
        }

        // Test with array that excludes zero selector
        if (shouldPassWithoutZero) {
            errAllow(selector, allowedErrorsWithoutZero, "fuzz zero excluded valid test");
        } else {
            vm.expectEmit(false, false, false, true);
            emit AssertFail("fuzz zero excluded invalid test");
            vm.expectRevert(PlatformTest.TestAssertFail.selector);
            errAllow(selector, allowedErrorsWithoutZero, "fuzz zero excluded invalid test");
        }
    }
