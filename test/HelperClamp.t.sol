...
contract TestHelperClamp is Test, HelperClamp {
    ...
    function test_clamp_uint256_cyclic_property() public {
        // Test that adding range size cycles back to same result for values outside range
        uint256 value = 25; // Outside range [10, 20]
        uint256 low = 10;
        uint256 high = 20;
        uint256 range = high - low + 1; // 11
    ...
    function test_clamp_int128_large_ranges() public {
        // Test with large ranges within int128 bounds
        int128 low = -10000;
        int128 high = 10000;
    ...
    function test_clamp_int128_cyclic_property() public {
        // Test that adding range size cycles back to same result
        int128 value = 15;
        int128 low = -10;
        int128 high = 10;
        int256 range = int256(high) - int256(low) + 1; // 21
        
        int128 result1 = this.clamp(value, low, high);
        // Test with smaller multiples to avoid overflow
    ...
    function test_clampLt_uint256_edge_cases() public {
        assertEq(this.clampLt(uint256(1), uint256(2)), uint256(1)); // Already < 2
        assertEq(this.clampLt(uint256(10), uint256(5)), uint256(0)); // 10 % 5 = 0
        assertEq(this.clampLt(uint256(11), uint256(5)), uint256(1)); // 11 % 5 = 1
    }

    function test_clampLt_uint256_overflow_cases() public {
        vm.expectRevert();
        this.clampLt(uint256(100), uint256(0)); // b - 1 underflows
    }

    function test_clampLt_int128_basic_behavior() public {
        assertEq(this.clampLt(int128(5), int128(10)), int128(5)); // Already < 10
        assertEq(this.clampLt(int128(-5), int128(10)), int128(-5)); // Already < 10
    ...
    function test_clampLt_int128_overflow_cases() public {
        vm.expectRevert();
        this.clampLt(int128(100), type(int128).min); // b - 1 underflows
    }

    /**
     * Tests for clampLte functions
     */
    function test_clampLte_uint256_basic_behavior() public {
        ...
        assertEq(this.clampLte(uint256(10), uint256(10)), uint256(10)); // Boundary case
    }
    ...
    function test_clampLte_uint256_edge_cases() public {
        ...
        assertEq(this.clampLte(type(uint256).max, uint256(50)), uint256(0)); // max % 51 = 0
    }

    function test_clampLte_int128_basic_behavior() public {
        assertEq(this.clampLte(int128(5), int128(10)), int128(5)); // Already <= 10
        assertEq(this.clampLte(int128(10), int128(10)), int128(10)); // Boundary case
        assertEq(this.clampLte(int128(-5), int128(10)), int128(-5)); // Already <= 10
    }

    /**
     * Tests for clampGt functions
     */
    function test_clampGt_uint256_basic_behavior() public {
        assertEq(this.clampGt(uint256(15), uint256(10)), uint256(15)); // Already > 10
    ...
    function test_clampGt_uint256_edge_cases() public {
        ...
        assertEq(this.clampGt(uint256(100), uint256(50)), uint256(100)); // Already > 50
    }

    function test_clampGt_uint256_overflow_cases() public {
        vm.expectRevert();
        this.clampGt(uint256(100), type(uint256).max); // b + 1 overflows
    }

    function test_clampGt_int128_basic_behavior() public {
        assertEq(this.clampGt(int128(15), int128(10)), int128(15)); // Already > 10
        assertEq(this.clampGt(int128(5), int128(10)), this.clamp(int128(5), int128(11), type(int128).max));
        assertEq(this.clampGt(int128(-5), int128(-10)), int128(-5)); // Already > -10
    }

    function test_clampGt_int128_overflow_cases() public {
        vm.expectRevert();
        this.clampGt(int128(100), type(int128).max); // b + 1 overflows
    }

    /**
     * Tests for clampGte functions
     */
    function test_clampGte_uint256_basic_behavior() public {
        assertEq(this.clampGte(uint256(15), uint256(10)), uint256(15)); // Already >= 10
        assertEq(this.clampGte(uint256(10), uint256(10)), uint256(10)); // Boundary case
    ...
    function test_clampGte_uint256_edge_cases() public {
        ...
        assertEq(this.clampGte(uint256(100), uint256(50)), uint256(100)); // Already >= 50
    ...
    function test_clampGte_uint256_with_logging() public {
        vm.expectEmit(true, true, true, true);
        emit Clamped("Clamping value 5 to 15"); // 5 wraps to 15 in range [10, max]
        this.clampGte(uint256(5), uint256(10));
    }

    function test_clampGte_int128_basic_behavior() public {
        assertEq(this.clampGte(int128(15), int128(10)), int128(15)); // Already >= 10
        assertEq(this.clampGte(int128(10), int128(10)), int128(10)); // Boundary case
        assertEq(this.clampGte(int128(5), int128(10)), this.clamp(int128(5), int128(10), type(int128).max));
        assertEq(this.clampGte(int128(-5), int128(-10)), int128(-5)); // Already >= -10
    }
    ...
    function testFuzz_clamp_uint256(uint256 value, uint256 low, uint256 high) public {
        ...
        // Test modular arithmetic property for values outside the range
    ...
    function test_clamp_uint256_distributive_property() public {
        // Test that clamping distributes over addition within range
        uint256 a = 5;
        uint256 b = 7;
        uint256 low = 10;
        uint256 high = 20;
        
        // Both values should map to same relative positions
        uint256 result_a = this.clamp(a, low, high);
        uint256 result_b = this.clamp(b, low, high);
        
        // The difference should be preserved modulo the range
        uint256 range = high - low + 1;
        assertEq((result_b - result_a + range) % range, (b - a) % range);
    }

    function test_clamp_int128_sign_preservation_property() public {
        // Test cases where sign relationships are preserved
        int128 pos_val = 50;
        int128 neg_val = -50;
        int128 low = -10;
        int128 high = 10;
    ...
    /*
     **************************************************************************
     * STRESS TESTS - Edge Cases and Extreme Values
     **************************************************************************
     */

    function test_clamp_uint256_consecutive_values() public {
        // Test consecutive values to ensure smooth wrapping
        uint256 low = 100;
        uint256 high = 105;
        
        uint256[] memory inputs = new uint256[](10);
        inputs[0] = 90; inputs[1] = 91; inputs[2] = 92; inputs[3] = 93; inputs[4] = 94;
        inputs[5] = 110; inputs[6] = 111; inputs[7] = 112; inputs[8] = 113; inputs[9] = 114;
    ...
    function test_clamp_int128_around_zero() public {
        // Test values around zero with symmetric range
        int128 low = -5;
        int128 high = 5;
        
        int128[] memory inputs = new int128[](11);
        inputs[0] = -10; inputs[1] = -9; inputs[2] = -8; inputs[3] = -7; inputs[4] = -6;
        inputs[5] = 0;
        inputs[6] = 6; inputs[7] = 7; inputs[8] = 8; inputs[9] = 9; inputs[10] = 10;
    ...
    function test_clamp_all_single_value_ranges() public {
        // Test various single-value ranges
        uint256[] memory values = new uint256[](5);
        values[0] = 0; values[1] = 1; values[2] = 100; values[3] = 1000; values[4] = type(uint256).max;
