...
library FuzzLibString {
    ...
    function intToString(int128 value) internal pure returns (string memory str) {
        uint128 absValue = value >= 0 ? uint128(value) : uint128(-value);
        str = toString(absValue);
    ...
    function toString(uint256 value) internal pure returns (string memory str) {
        ...
        function intToString(uint128 value) internal pure returns (string memory str) {
        /// @solidity memory-safe-assembly
        assembly {
            // The maximum value of a uint256 contains 78 digits (1 byte per digit), but we allocate 160 bytes
            // to keep the free memory pointer word aligned. We'll need 1 word for the length, 1 word for the
    ...
    function intToString(address value) internal pure returns (string memory str) {
        bytes memory s = new bytes(40);
        for (uint128 i = 0; i < 20; i++) {
            bytes1 b = bytes1(
                uint8(uint128(uint160(value)) / (2**(8 * (19 - i))))
            );
            bytes1 hi = bytes1(uint8(b) / 16);
            bytes1 lo = bytes1(uint8(b) - 16 * uint8(hi));
            s[2 * i] = char(hi);
